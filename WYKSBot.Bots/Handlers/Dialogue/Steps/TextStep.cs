...
using WYKSBot.Bots.Dialogue.Steps;

namespace WYKSBot.Bots.Handlers.Dialogue.Steps
{
        ...
        public override async Task<bool> ProcessStep(DiscordClient client, DiscordChannel channel, DiscordUser user)
        {
            ...
            while(true)
            {
                ...
                if(_minLength.HasValue)
                {
                    ...
                    {
                        await TryAgain(channel, $"Your input is {_minLength.Value - messageResult.Result.Content.Length} characters are too short").ConfigureAwait(false);
                        continue;
                    }
                }
                ...
                if (_maxLength.HasValue)
                {
                    ...
                    {
                        await TryAgain(channel, $"Your input is {_maxLength.Value - messageResult.Result.Content.Length} characters are too short").ConfigureAwait(false);
                        continue;
                    }
                }
